...
def get_active_players(game: Game | str | None, region: Region | str | None, season_start: date | None=None, season_end: date | None=None, minimum_events_to_count: int=1, series_to_exclude: Collection[str] | None=None, only_count_locals: bool=True, only_count_main_bracket: bool=True) -> Collection[Player]:
	...
	for player in players_to_check:
		for result in Result.results_for_player(player, season_start, season_end):
			...
			event = result.event
			if game and (event.game.short_name != (game.short_name if isinstance(game, Game) else game)):
				continue
			if only_count_main_bracket and (event.is_redemption_bracket or event.is_side_bracket or event.type != EventType.Singles):
				continue
