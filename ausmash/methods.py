...
def get_players_of_character(char: 'Character') -> Collection[CharacterPlayer]:
	...
	Returns:
		Collection of CharacterPlayer objects"""
...
def get_videos_of_player_in_game(player: Player, game: Game | str) -> Sequence[PocketVideo]:
	...
	Returns:
		List of PocketVideo objects (newest to oldest?)"""
	if isinstance(game, str):
		game = Game(game)
...
def get_matches_of_player_in_game(player: Player, game: Game | str) -> Sequence[PocketMatchWithPOV]:
	...
	Returns:
		List of PocketMatchWithPOV objects, from the player's perspective"""
	if isinstance(game, str):
		game = Game(game)
	return __flatten_pocket_match_array(
...
def get_active_players(
	...
	*,
	...
	Args:
		game: Only count players who entered events for a particular game, or None for all Smash players
		region: Region to get active players from, or None for anywhere
		season_start: Count tournaments starting from this date
		season_end: Count tournaments up until this date, so you can get active players for a PR season
		minimum_events_to_count: Require players to have attended this many tournaments to be considered active
		series_to_exclude: If specified, list of TournamentSeries names to not count, e.g. an arcadian series if that makes sense for whatever you are using this method for
		only_count_locals: Don't count tournaments outside of region
		only_count_main_bracket: Don't count a player as attending a tournament if they only entered redemption, a side bracket, or doubles

	Returns:
		Players meeting the criteria, not necessarily sorted
	"""
