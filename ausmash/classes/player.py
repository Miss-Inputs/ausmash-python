...
class Player(Resource):
	...
	def all(cls, region: Region | str | None = None) -> Collection['Player']:
		...
		if region:
			...
			return {
				cls(p).updated_copy({'Region': region._data})
				for p in call_api_json(f'players/byregion/{region.short_name}')
	...
	@classmethod
	def reverse_lookup_start_gg_player_id(
		cls,
		player_id: str | int,
		player_name_hint: str | None = None,
		player_region_hint: Region | str | None = None,
		*,
		skip_searching_all: bool = False,
	) -> 'Player | None':
		...
		if player_name_hint:
			...
			name_result = next(
				(p for p in cls.search(player_name_hint) if str(p.start_gg_player_id) == player_id),
				None,
			)
			if name_result:
				return name_result
		if player_region_hint:
			region_result = next(
				(p for p in cls.all(player_region_hint) if str(p.start_gg_player_id) == player_id),
				None,
			)
			if region_result:
	...
	def compare_common_winrates(
		self,
		game: Game,
		other: 'Player',
		start_date: date | None = None,
		end_date: date | None = None,
	) -> tuple[Sequence['WinRate'], Sequence['WinRate']]:
		params = {}
		if start_date:
		...
		return WinRate.wrap_many(response['Player1WinRates']), WinRate.wrap_many(
			response['Player2WinRates']
		)

	def compare_common_results(
		self,
		game: Game,
		other: 'Player',
		start_date: date | None = None,
		...
		if end_date:
			params['endDate'] = end_date.isoformat()
		return tuple(
			(Event(result['Event']), result['Player1Result'], result['Player1Result'])
			for result in call_api_json(f'compare/{game.id}/{self.id}/{other.id}/results', params)[
				'Results'
			]
		)
